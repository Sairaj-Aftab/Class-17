...
stu_form.addEventListener('submit', function(e){
    ...
    const photos = this.querySelector('.result input[placeholder="Photo Link"]').value;
    ...
    if (name == '' || roll == '' || stuClass == '' || photos == '' || gender == '' || bangla == '' || english == '' | math == '' || social == '' || science == '' || religion == '') {
        alert()
    } else {
...
function allStudent() {
    ...
    allData.map((data, index) => {
        ...
        <td>${data.stuClass}</td>
        <td>${data.gender}</td>
        <td>${result.tgpa(result.sgpa(data.bangla) , result.sgpa(data.english) , result.sgpa(data.math) , result.sgpa(data.social) , result.sgpa(data.science) , result.sgpa(data.religion)).totalgrade}</td>
